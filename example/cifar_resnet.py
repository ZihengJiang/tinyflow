...
# x = tf.nn.batch_normalization(x)
conv1 = tf.nn.conv2d(x, num_filter=filter_num[0], ksize=[1, 5, 5, 1], padding='SAME')
tanh1 = tf.tanh(conv1)

# res = resnet(tanh1, 1, filter_num[0], filter_num[-1])
res = tanh1
...
batch_xs, batch_ys = cifar.test.next_batch(batch_size)
print(sess.run(accuracy, feed_dict={x: batch_xs, label: batch_ys}))
